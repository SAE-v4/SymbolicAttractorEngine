...
export class SpiralGateChamber extends ChamberBase {
  ...
  render(alpha: number) {
    ...
    if (rippleA > 0.01) {
      ...
      const r1 =
        Math.min(w, h) * (0.15 + 0.6 * (1 - this.fx.overlayAlpha(0.6))); // expands as ripple decays
