...
export class FlowToyChamber extends HTMLElement {
  ...
  private readonly DRAG = 0.985;       // velocity decay
  ...
  private resetWorld() {
    ...
    this.centres = [
      ...
      { id: "root", p: { x: cx - R, y: cy + R * 0.4 }, w: 1.0, spin: 0.70, radial: -0.10, sigma: Math.min(this.w, this.h) * 0.55, phase: 0.00 },
      // Shoots (slightly outward)
      { id: "shoot", p: { x: cx + R * 0.9, y: cy - R * 0.5 }, w: 1.0, spin: 0.65, radial: +0.10, sigma: Math.min(this.w, this.h) * 0.55, phase: 0.33 },
      // Branching (neutral)
      { id: "branch", p: { x: cx + R * 0.1, y: cy + R * 0.9 }, w: 0.9, spin: 0.80, radial: 0.00, sigma: Math.min(this.w, this.h) * 0.45, phase: 0.66 },
      // Weaving (slightly inward, lower spin)
      { id: "weave", p: { x: cx - R * 0.2, y: cy - R * 0.9 }, w: 0.9, spin: 0.60, radial: -0.05, sigma: Math.min(this.w, this.h) * 0.45, phase: 0.85 },
    ];
    ...
    for (let i = 0; i < this.DOTS; i++) {
      ...
      const rr = Math.random() * (Math.min(this.w, this.h) * 0.06);
  ...
  private fieldAt(p: Vec2): Vec2 {
    ...
    for (const c of this.centres) {
      ...
      const spin = c.spin;
      const radial = c.radial + breathInwardBias;
