import vertSrc from "./shaders/sky.vert.gsl?raw";
import fragSrc from "./shaders/sky.frag.gsl?raw";
...
export class SkyGLRenderer {
  ...
  private u_flow: WebGLUniformLocation | null;
  private flow = 0;
  ...
  setFlow(x:number){ this.flow = Math.min(1, Math.max(0, x)); }
  ...
  render(timeSec: number) {
    ...
    // NEW — ring uniforms
    const dpr = this.canvas.clientWidth
    ...
    gl.uniform1f(this.u_flow!, this.flow);
